...
impl SyncEngine {
    ...
    pub async fn init_sync<C: MailClient>(
        ...
        for local in local_folders {
            if !remote_ids.contains(&local.remote_id) {
                println!(
                    "[Sync] Pruning orphaned folder: {} ({})",
                    local.name, local.remote_id
                );
    ...
    pub async fn prune_deleted_emails<C: MailClient>(
        ...
        let mut fetch_limit = last_uid.saturating_sub(first_uid).saturating_add(1) as usize;
        if fetch_limit < 50 {
            fetch_limit = 50;
        }
        if fetch_limit > 1000 {
    ...
    pub async fn sync_history_step<C: MailClient>(
        ...
        let sync_enabled = self
            .db
            .get_setting("background_sync_history")
            .await?
            .map(|v| v == "true")
            .unwrap_or(true);
    ...
    async fn store_synced_email(
        &self,
        folder_id: &str,
        email: &super::traits::EmailSummary,
    ) -> Result<()> {
        if let Some(msg_id) = &email.message_id {
            if let Ok(Some(local_uid)) = self.db.find_email_by_message_id(folder_id, msg_id).await {
                if local_uid.starts_with("local-sent-") {
                    let _ = self
                        .db
                        .update_remote_id(folder_id, &local_uid, &email.uid)
                        .await;
                }
            }
        }
...
mod tests {
    ...
    async fn test_sync_no_new_emails() {
        ...
        impl MailClient for EmptyClient {
            ...
            async fn rename_folder(&mut self, _: &str, _: &str) -> Result<()> {
                Ok(())
            }
            async fn delete_folder(&mut self, _: &str) -> Result<()> {
                Ok(())
            }
    ...
    impl MailClient for PruneClient {
        ...
        async fn get_emails_since(&mut self, _: &str, _: u32) -> Result<Vec<EmailSummary>> {
            Ok(vec![])
        }
        async fn get_emails_before(
            &mut self,
    ...
    async fn test_prune_deleted_emails_removes_missing_uids() {
        ...
        let acct_id = db
            .upsert_account("prune@test.com", None, "h", 993, true, "h", 465, true)
            .await
            .unwrap();
        let folder_id = db
            .upsert_folder(&acct_id, "INBOX", "Inbox", 0, Some("INBOX"))
