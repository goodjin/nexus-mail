...
use crate::core::traits::{
    self, AccountInfo, AttachmentInfo, EmailHeader, EmailSummary, FolderInfo, MailClient,
    MailSender, SendEmailRequest,
};
use anyhow::Result;
...
fn classify_account_status(message: &str) -> &'static str {
...
const MAX_CUSTOM_FOLDER_LEN: usize = 64;
const RESERVED_FOLDER_NAMES: [&str; 14] = [
    "inbox",
    "sent",
    "drafts",
    "trash",
...
fn is_reserved_folder_name(name: &str) -> bool {
    ...
    RESERVED_FOLDER_NAMES.iter().any(|value| *value == normalized)
}

fn normalize_custom_folder_name(name: &str) -> Result<String, String> {
    let trimmed = name.trim();
    if trimmed.is_empty() {
        return Err("Folder name cannot be empty".to_string());
    }
    if trimmed.len() > MAX_CUSTOM_FOLDER_LEN {
        return Err(format!(
            "Folder name must be {} characters or less",
            MAX_CUSTOM_FOLDER_LEN
        ));
    }
    if trimmed.contains('/') || trimmed.contains('\\') {
        return Err("Folder name cannot include '/' or '\\'".to_string());
    }
    if is_reserved_folder_name(trimmed) {
        return Err("Folder name is reserved for system folders".to_string());
    }
    Ok(trimmed.to_string())
}

fn ensure_custom_folder(folder: &FolderInfo) -> Result<(), String> {
    if folder.system_role.is_some() || is_reserved_folder_name(&folder.name) {
        return Err("System folders cannot be modified".to_string());
    }
    Ok(())
}
...
pub struct AppSettingsPayload {
    ...
    pub background_sync_history: bool,
    pub theme: String,
    pub confirm_before_delete: bool,
    pub download_directory: String,
    pub remote_image_policy: String,
...
fn build_app_settings(settings: &std::collections::HashMap<String, String>) -> AppSettingsPayload {
    AppSettingsPayload {
        auto_download_attachments: parse_bool_setting(settings, "auto_download_attachments", false),
        background_sync_history: parse_bool_setting(settings, "background_sync_history", true),
        theme: settings
            .get("theme")
            .cloned()
            .unwrap_or_else(|| "system".to_string()),
        confirm_before_delete: parse_bool_setting(settings, "confirm_before_delete", true),
        download_directory: settings
            .get("download_directory")
            .cloned()
            .unwrap_or_default(),
        remote_image_policy: settings
            .get("remote_image_policy")
            .cloned()
            .unwrap_or_else(|| "ask".to_string()),
        search_history_limit: settings
            .get("search_history_limit")
            .and_then(|value| value.parse::<u32>().ok())
            .unwrap_or(10),
    }
}

fn validate_app_settings(settings: &AppSettingsPayload) -> Result<(), String> {
    if !matches!(settings.theme.as_str(), "light" | "dark" | "system") {
        return Err(format!("Invalid theme mode: {}", settings.theme));
    }
    if !matches!(
        settings.remote_image_policy.as_str(),
        "always" | "ask" | "never"
    ) {
        return Err(format!(
            "Invalid remote image policy: {}",
            settings.remote_image_policy
        ));
    }
    if settings.search_history_limit == 0 || settings.search_history_limit > 50 {
        return Err("search_history_limit must be between 1 and 50".to_string());
    }
    Ok(())
}
...
fn find_folder_by_role<'a>(folders: &'a [FolderInfo], role: &str) -> Option<&'a FolderInfo> {
    folders.iter().find(|folder| {
        folder.system_role.as_deref() == Some(role) || folder.remote_id.eq_ignore_ascii_case(role)
    })
}
...
async fn load_search_results(
    ...
    for id in ids {
        ...
        .await
        {
            let folder_matches = if let Some(account_id) = account_id {
...
async fn load_folder_remote_id_for_account(
    db: &Database,
    account_id: &str,
    folder_id: &str,
) -> Result<String, String> {
    sqlx::query_as::<_, (String,)>("SELECT remote_id FROM folders WHERE id = ? AND account_id = ?")
        .bind(folder_id)
        .bind(account_id)
        .fetch_optional(&db.pool)
        .await
        ...
        .map(|row| row.0)
        .ok_or_else(|| "Folder not found for account".to_string())
}
...
pub async fn update_account_details(
    ...
    if sync_interval == 0 || sync_interval > 1440 {
    ...
    db.set_account_sync_preferences(&email, sync_enabled, sync_interval)
        .await
        .map_err(|e: anyhow::Error| e.to_string())?;
    Ok(())
}
...
#[tauri::command]
pub async fn update_account_password(email: String, password: String) -> Result<(), String> {
    SecurityService::set_password(&email, &password).map_err(|e: anyhow::Error| e.to_string())
}
...
pub async fn sync_account(
    ...
    let result: Result<String, String> = async {
        ...
        let password =
            SecurityService::get_password(&email).map_err(|e: anyhow::Error| e.to_string())?;
        ...
        if !db.has_any_system_roles(&account.id).await.unwrap_or(false) {
            crate::info!(
                "Legacy folder data detected for {}, performing one-time cleanup",
                email
            );
        ...
        let db_folders_initial = db
            .get_folders_for_account(&account.id)
            .await
            .map_err(|e| e.to_string())?;

        let remote_ids: std::collections::HashSet<String> =
            remote_folders.iter().map(|f| f.remote_id.clone()).collect();
        let _local_ids: std::collections::HashSet<String> = db_folders_initial
            .iter()
            .map(|f| f.remote_id.clone())
            .collect();

        // 1. Add/Update Folders
        for remote_f in remote_folders {
            crate::info!(
                "Syncing remote folder: {} (Role: {:?})",
                remote_f.remote_id,
                remote_f.system_role
            );
            let _ = db
                .upsert_folder(
                    &account.id,
        ...
        for db_folder in db_folders {
            ...
            if let Err(e) = engine
                .sync_emails::<RealImapClient>(&mut client, &folder.id, &folder.remote_id)
                .await
            {
            ...
            if let Err(e) = engine
                .prune_deleted_emails::<RealImapClient>(&mut client, &folder.id, &folder.remote_id)
                .await
            {
        ...
        if has_errors {
            Ok(format!(
                "Account {} synced with some folder errors (check logs)",
                email
            ))
        } else {
    ...
    match &result {
        ...
        Err(message) => {
            let _ = db
                .set_account_runtime_state(
                    &email,
                    classify_account_status(message),
                    Some(message),
                    None,
                )
                .await;
...
mod tests {
    ...
    fn test_resolve_account_test_password_from_argument() {
        let password =
            resolve_account_test_password("demo@nexus-mail.com", Some("secret".to_string()))
                .unwrap();
    ...
    fn test_split_header_addresses_trims() {
        let addresses = split_header_addresses(Some("a@test.com, b@test.com  ,".to_string()));
        assert_eq!(
            addresses,
    ...
    #[test]
    fn test_normalize_custom_folder_name_rejects_reserved() {
        let err = normalize_custom_folder_name("Inbox").unwrap_err();
        assert!(err.contains("reserved"));
    }

    #[test]
    fn test_normalize_custom_folder_name_rejects_empty() {
        let err = normalize_custom_folder_name("   ").unwrap_err();
        assert!(err.contains("cannot be empty"));
    }

    #[test]
    fn test_ensure_custom_folder_rejects_system_role() {
        ...
        let err = ensure_custom_folder(&folder).unwrap_err();
        assert!(err.contains("System folders"));
    }

    #[tokio::test]
    async fn test_load_folder_remote_id_for_account_allows_matching_folder() {
        ...
        let account_id = db
            ...
            .unwrap();
        let folder_id = db
            .upsert_folder(&account_id, "INBOX", "Inbox", 0, Some("INBOX"))
            .await
            .unwrap();

        let remote_id = load_folder_remote_id_for_account(&db, &account_id, &folder_id)
            .await
            .unwrap();

        assert_eq!(remote_id, "INBOX");
    }

    #[tokio::test]
    async fn test_load_folder_remote_id_for_account_rejects_cross_account_folder() {
        let db = create_test_db().await;
        let owner_account_id = db
            .upsert_account(
                "owner@test.com",
                None,
                "imap.owner.test",
        ...
        let attacker_account_id = db
            ...
            .await
            .unwrap();
        let victim_folder_id = db
            .upsert_folder(&owner_account_id, "INBOX", "Inbox", 0, Some("INBOX"))
            .await
            .unwrap();

        let err = load_folder_remote_id_for_account(&db, &attacker_account_id, &victim_folder_id)
            .await
            .unwrap_err();
...
pub async fn get_email_details(
    ...
    let folder_remote_id = load_folder_remote_id_for_account(&db, &account.id, &folder_id).await?;

    let cached_row: Option<(
        String,
        Option<String>,
        Option<String>,
        Option<String>,
    ...
    if let Some((email_id, body_html, body_text, headers_json, cached_at)) = cached_row.as_ref() {
        if let Some(cached_at) = cached_at {
            ...
            if is_fresh && (body_html.is_some() || body_text.is_some()) && headers_json.is_some() {
                ...
                .into_iter()
                .map(
                    |(id, filename, mime_type, size, content_id, is_inline)| AttachmentInfo {
                        id,
                        filename,
                        mime_type,
                        size: size as usize,
    ...
    let password =
        SecurityService::get_password(&account_email).map_err(|e: anyhow::Error| e.to_string())?;
    ...
    let details = client
        .get_email_details(&folder_remote_id, &uid)
        .await
        .map_err(|e| e.to_string())?;
    ...
    let attachments = if let Some(email_id) = email_id {
        ...
        let fts_body = body_text.as_deref().or(body_html.as_deref()).unwrap_or("");
        let _ = sqlx::query(
...
pub async fn get_folders(
    ...
    .map(|rows| {
        rows.into_iter()
            .map(
                |(id, remote_id, name, unread_count, system_role)| FolderInfo {
                    id,
                    name,
                    remote_id,
...
pub async fn create_folder(
    ...
    let normalized = normalize_custom_folder_name(&name)?;
    if db
        .folder_name_exists(&account.id, &normalized)
        .await
        .map_err(|e| e.to_string())?
    {
        return Err("Folder name already exists".to_string());
    }

    let remote_id = utf7_imap::encode_utf7_imap(normalized.clone());
    if db
        .folder_remote_id_exists(&account.id, &remote_id)
        .await
        .map_err(|e| e.to_string())?
    {
    ...
    client
        ...
        .map_err(|e| e.to_string())?;
    client
        .create_folder(&remote_id)
        .await
        .map_err(|e| e.to_string())?;

    let folder_id = db
        .upsert_folder(&account.id, &remote_id, &normalized, 0, None)
        .await
        .map_err(|e| e.to_string())?;
...
pub async fn rename_folder(
    ...
    let folder = db
        .get_folder_for_account(&account.id, &folder_id)
        .await
        .map_err(|e| e.to_string())?
        .ok_or_else(|| "Folder not found".to_string())?;

    ensure_custom_folder(&folder)?;
    let normalized = normalize_custom_folder_name(&new_name)?;
    ...
    let new_remote_id = utf7_imap::encode_utf7_imap(normalized.clone());
    if db
        .folder_remote_id_exists(&account.id, &new_remote_id)
        .await
        .map_err(|e| e.to_string())?
    {
        return Err("Folder already exists".to_string());
    ...
    client
        ...
        .map_err(|e| e.to_string())?;
    client
        .rename_folder(&folder.remote_id, &new_remote_id)
        .await
        .map_err(|e| e.to_string())?;

    db.update_folder_metadata(&folder.id, &new_remote_id, &normalized)
        .await
        .map_err(|e| e.to_string())?;
...
pub async fn delete_folder(
    ...
    client
        ...
        .map_err(|e| e.to_string())?;
    client
        .delete_folder(&folder.remote_id)
        .await
        .map_err(|e| e.to_string())?;
...
pub async fn search_emails(
    ...
    load_search_results(&db, Some(&account.id), ids).await
}
...
pub async fn search_emails_with_filters(
    ...
    let start_date = filters
        .as_ref()
        .and_then(|filter| filter.start_date.as_deref());
    let end_date = filters
        .as_ref()
        .and_then(|filter| filter.end_date.as_deref());
    let has_attachments = filters.as_ref().and_then(|filter| filter.has_attachments);
    let account_scope = filters
        .as_ref()
        .and_then(|filter| filter.account_scope.as_deref());
    let folder_ids = filters
        .as_ref()
        .and_then(|filter| filter.folder_ids.as_ref())
        .map(|values| values.as_slice());
    let account_filter = if matches!(account_scope, Some("all_accounts")) {
        None
    } else {
    ...
    load_search_results(&db, account_filter, ids).await
}
...
pub async fn get_search_history(
    ...
    db.get_search_history(&account.id)
        ...
        .map(|rows| {
            rows.into_iter()
                .map(|(query, last_used_at)| SearchHistoryEntry {
                    query,
                    last_used_at,
                })
                .collect()
...
pub async fn get_attachment(
    ...
    let data = client
        .get_attachment(&folder_remote_id, &uid, part_id)
        .await
        .map_err(|e| e.to_string())?;
...
pub async fn update_email_flag(
    ...
    client
        .set_flag(&folder_remote_id, &uid, &flag, value)
        .await
        .map_err(|e| e.to_string())?;
...
pub async fn apply_email_action(
    ...
    let folder_remote_id = load_folder_remote_id_for_account(&db, &account.id, &request.folder_id)
        .await
        .map_err(|e| EmailActionError::new("folder_not_found", e))?;
    ...
    for uid in &request.uids {
        let result = match request.action {
            EmailAction::MarkRead => {
                if let Err(e) = client
                    .set_flag(&folder_remote_id, uid, "\\Seen", true)
                    .await
                {
                    Err(e.to_string())
                } else {
            ...
            EmailAction::MarkUnread => {
                if let Err(e) = client
                    .set_flag(&folder_remote_id, uid, "\\Seen", false)
                    .await
                {
                    Err(e.to_string())
                } else {
            ...
            EmailAction::Flag => {
                if let Err(e) = client
                    .set_flag(&folder_remote_id, uid, "\\Flagged", true)
                    .await
                {
                    Err(e.to_string())
                } else {
            ...
            EmailAction::Unflag => {
                if let Err(e) = client
                    .set_flag(&folder_remote_id, uid, "\\Flagged", false)
                    .await
                {
                    Err(e.to_string())
                } else {
            ...
            EmailAction::Archive => {
                if let Err(e) = client
                    .set_flag(&folder_remote_id, uid, "\\Archived", true)
                    .await
                {
                    Err(e.to_string())
                } else {
            ...
            EmailAction::Delete => {
                if let Err(e) = client.delete_email(&folder_remote_id, uid).await {
                    Err(e.to_string())
...
pub async fn move_emails(
    ...
    let source_remote_id =
        load_folder_remote_id_for_account(&db, &account.id, &source_folder_id).await?;

    let target_remote_id =
        load_folder_remote_id_for_account(&db, &account.id, &target_folder_id).await?;
    ...
    for uid in &uids {
        match client
            .move_email(&source_remote_id, uid, &target_remote_id)
            .await
        {
            Ok(()) => {
...
pub async fn delete_email(
    ...
    client
        .delete_email(&folder_remote_id, &uid)
        .await
        .map_err(|e| e.to_string())?;
...
pub async fn reset_database(db: State<'_, Database>) -> Result<(), String> {
    ...
    sqlx::query("DELETE FROM accounts")
        ...
        .await
        .map_err(|e| e.to_string())?;
    sqlx::query("DELETE FROM emails_fts")
        .execute(&db.pool)
        .await
        .map_err(|e| e.to_string())?;
    Ok(())
...
#[tauri::command]
pub async fn get_app_settings(db: State<'_, Database>) -> Result<AppSettingsPayload, String> {
    let settings = db.get_all_settings().await.map_err(|e| e.to_string())?;
    Ok(build_app_settings(&settings))
}
...
pub async fn update_app_settings(
    ...
    db.set_setting(
        ...
        &settings.auto_download_attachments.to_string(),
    )
    .await
    .map_err(|e| e.to_string())?;
    db.set_setting(
        "background_sync_history",
        &settings.background_sync_history.to_string(),
    )
    .await
    .map_err(|e| e.to_string())?;
    db.set_theme_mode(&settings.theme)
        .await
        .map_err(|e| e.to_string())?;
    db.set_setting(
    ...
    .await
    .map_err(|e| e.to_string())?;
    db.set_setting("download_directory", &settings.download_directory)
        .await
        .map_err(|e| e.to_string())?;
    db.set_setting("remote_image_policy", &settings.remote_image_policy)
        .await
        .map_err(|e| e.to_string())?;
    db.set_setting(
        "search_history_limit",
        &settings.search_history_limit.to_string(),
    )
    .await
    .map_err(|e| e.to_string())
}
...
#[tauri::command]
pub async fn get_theme_mode(db: State<'_, Database>) -> Result<String, String> {
    db.get_theme_mode()
        .await
...
#[tauri::command]
pub async fn update_theme_mode(db: State<'_, Database>, mode: String) -> Result<(), String> {
    db.set_theme_mode(&mode).await.map_err(|e| e.to_string())
}

#[tauri::command]
pub async fn get_shortcuts(db: State<'_, Database>) -> Result<String, String> {
    db.get_shortcuts().await.map_err(|e| e.to_string())
}

#[tauri::command]
pub async fn update_shortcuts(db: State<'_, Database>, shortcuts: String) -> Result<(), String> {
    db.set_shortcuts(&shortcuts)
        .await
        .map_err(|e| e.to_string())
}
...
pub async fn dev_seed_data(db: State<'_, Database>) -> Result<String, String> {
    ...
    let inbox_id = db
        .upsert_folder(&acct_id, "INBOX", "Inbox", 95, Some("INBOX"))
        .await
        .map_err(|e| e.to_string())?;
    db.upsert_folder(&acct_id, "SENT", "Sent", 0, Some("SENT"))
        .await
        .map_err(|e| e.to_string())?;
    db.upsert_folder(&acct_id, "TRASH", "Trash", 0, Some("TRASH"))
        .await
        .map_err(|e| e.to_string())?;
    db.upsert_folder(&acct_id, "DRAFTS", "Drafts", 2, Some("DRAFTS"))
        .await
        .map_err(|e| e.to_string())?;
    db.upsert_folder(&acct_id, "SPAM", "Spam", 5, Some("SPAM"))
        .await
        .map_err(|e| e.to_string())?;

    for i in 1..=100 {
        let is_unread = i <= 95;
        let flags = if is_unread {
            vec![]
        } else {
            vec!["\\Seen".to_string()]
        };
        ...
        db.upsert_email(&inbox_id, &summary)
            .await
            .map_err(|e| e.to_string())?;
    }

    // 更新未读数
    db.update_folder_unread_count(&inbox_id)
        .await
        .map_err(|e| e.to_string())?;
