    ...
    switch (cmd) {
      ...
      case "get_emails":
        ...
        return Array.from({ length: 100 }, (_, i) => {
          ...
          const subject = isDemo && id == 99 ? "SECURITY TEST" : baseSubject;
          return { 
            uid: String(id), 
            subject, 
            from: isDemo ? `sender-${id}@mock.com` : `${folderKey}-${id}@${accountEmail.split("@")[1] || "mock.com"}`, 
            date: "Wed, 25 Mar 2026 10:19:36 +0800", 
            snippet: isDemo && id == 99 ? "Malicious payload test" : (isDemo ? `Mock content for message ${id}` : `Mock content for ${accountEmail} ${folderKey} ${id}`),
