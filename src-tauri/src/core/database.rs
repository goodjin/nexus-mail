...
const SETTING_CONFIRM_BEFORE_DELETE: &str = "confirm_before_delete";
const SETTING_DOWNLOAD_DIRECTORY: &str = "download_directory";
const SETTING_REMOTE_IMAGE_POLICY: &str = "remote_image_policy";
const SETTING_SEARCH_HISTORY_LIMIT: &str = "search_history_limit";
const SETTING_SHORTCUTS: &str = "shortcuts";

const DEFAULT_SETTINGS: [(&str, &str); 8] = [
    (SETTING_AUTO_DOWNLOAD_ATTACHMENTS, "false"),
    (SETTING_BACKGROUND_SYNC_HISTORY, "true"),
    (SETTING_THEME_MODE, "system"),
    (SETTING_CONFIRM_BEFORE_DELETE, "true"),
    (SETTING_DOWNLOAD_DIRECTORY, ""),
    (SETTING_REMOTE_IMAGE_POLICY, "ask"),
    (SETTING_SEARCH_HISTORY_LIMIT, "10"),
    (SETTING_SHORTCUTS, "{}"),
];
...
impl Database {
    ...
    pub async fn init_tables(&self) -> Result<()> {
        ...
        // 兼容性：如果旧表没有这些列，尝试添加
        let _ =
            sqlx::query("ALTER TABLE accounts ADD COLUMN imap_use_tls BOOLEAN NOT NULL DEFAULT 1")
                .execute(&self.pool)
                .await;
        let _ = sqlx::query(
        ...
        .await;
        let _ =
            sqlx::query("ALTER TABLE accounts ADD COLUMN smtp_use_tls BOOLEAN NOT NULL DEFAULT 1")
                .execute(&self.pool)
                .await;
        let _ = sqlx::query(
        ...
        .await;
        let _ =
            sqlx::query("ALTER TABLE accounts ADD COLUMN sync_enabled BOOLEAN NOT NULL DEFAULT 1")
                .execute(&self.pool)
                .await;
        let _ = sqlx::query(
            "ALTER TABLE accounts ADD COLUMN sync_interval INTEGER NOT NULL DEFAULT 15",
        )
        .execute(&self.pool)
        .await;
        let _ = sqlx::query("ALTER TABLE accounts ADD COLUMN last_sync INTEGER")
            .execute(&self.pool)
            .await;
        let _ =
            sqlx::query("ALTER TABLE accounts ADD COLUMN status TEXT NOT NULL DEFAULT 'normal'")
                .execute(&self.pool)
                .await;
        let _ = sqlx::query("ALTER TABLE accounts ADD COLUMN last_error TEXT")
            .execute(&self.pool)
            .await;
        ...
        let _ = sqlx::query("ALTER TABLE attachments ADD COLUMN content_id TEXT")
            ...
            .await;
        let _ =
            sqlx::query("ALTER TABLE attachments ADD COLUMN is_inline BOOLEAN NOT NULL DEFAULT 0")
                .execute(&self.pool)
                .await;
    ...
    pub async fn set_account_sync_preferences(
        &self,
        email: &str,
        sync_enabled: bool,
        sync_interval: u16,
        ...
        sqlx::query(
            "UPDATE accounts
             SET sync_enabled = ?, sync_interval = ?
             WHERE email = ?",
        )
        .bind(sync_enabled)
    ...
    pub async fn set_account_runtime_state(
        &self,
        email: &str,
        status: &str,
        ...
        sqlx::query(
            "UPDATE accounts
             SET status = ?, last_error = ?, last_sync = COALESCE(?, last_sync)
             WHERE email = ?",
        )
        .bind(status)
        .bind(last_error)
    ...
    pub async fn upsert_folder(
        ...
        if let Some(row) = existing {
            ...
            sqlx::query(
                "UPDATE folders SET name = ?, unread_count = ?, system_role = ? WHERE id = ?",
            )
            .bind(name)
    ...
    pub async fn get_folder_for_account(
        &self,
        account_id: &str,
        folder_id: &str,
    ...
    pub async fn folder_name_exists(&self, account_id: &str, name: &str) -> Result<bool> {
        let row: (i64,) = sqlx::query_as(
            "SELECT COUNT(*) FROM folders WHERE account_id = ? AND LOWER(name) = LOWER(?)",
        )
        .bind(account_id)
        .bind(name)
        .fetch_one(&self.pool)
    ...
    pub async fn folder_remote_id_exists(
        &self,
        account_id: &str,
        remote_id: &str,
    ) -> Result<bool> {
        let row: (i64,) = sqlx::query_as(
            "SELECT COUNT(*) FROM folders WHERE account_id = ? AND remote_id = ?",
        )
        .bind(account_id)
        .bind(remote_id)
        .fetch_one(&self.pool)
    ...
    pub async fn update_folder_metadata(
        ...
        sqlx::query("UPDATE folders SET remote_id = ?, name = ? WHERE id = ?")
            .bind(remote_id)
            .bind(name)
            .bind(folder_id)
            .execute(&self.pool)
    ...
    pub async fn find_email_by_message_id(
        &self,
        folder_id: &str,
        message_id: &str,
    ) -> Result<Option<String>> {
        let res: Option<(String,)> =
            sqlx::query_as("SELECT remote_id FROM emails WHERE folder_id = ? AND message_id = ?")
                .bind(folder_id)
                .bind(message_id)
                .fetch_optional(&self.pool)
                .await?;
    ...
    pub async fn search_emails_filtered(
        ...
        if account_id.is_some() {
            sql.push_str(" AND folders.account_id = ?");
        }
        ...
        if let Some(has_attachments) = has_attachments {
            if has_attachments {
                sql.push_str(
                    " AND EXISTS(SELECT 1 FROM attachments WHERE attachments.email_id = emails.id)",
                );
            } else {
    ...
    pub async fn get_account_by_email(&self, email: &str) -> Result<Option<AccountInfo>> {
        let row: Option<(String, String, Option<String>, String, i64, bool, String, i64, bool, bool, i64, Option<i64>, String, Option<String>)> = sqlx::query_as(
            "SELECT id, email, display_name, imap_host, imap_port, imap_use_tls, smtp_host, smtp_port, smtp_use_tls, sync_enabled, sync_interval, last_sync, status, last_error FROM accounts WHERE email = ?"
        )
        .bind(email)
        .fetch_optional(&self.pool)
        .await?;
    ...
    pub async fn find_account_by_domain(&self, domain: &str) -> Result<Option<AccountInfo>> {
        ...
        let row: Option<(String, String, Option<String>, String, i64, bool, String, i64, bool, bool, i64, Option<i64>, String, Option<String>)> =
            sqlx::query_as(
                "SELECT id, email, display_name, imap_host, imap_port, imap_use_tls, smtp_host, smtp_port, smtp_use_tls, sync_enabled, sync_interval, last_sync, status, last_error
    ...
    pub async fn get_all_uids_in_folder(&self, folder_id: &str) -> Result<Vec<String>> {
        let rows: Vec<(String,)> =
            sqlx::query_as("SELECT remote_id FROM emails WHERE folder_id = ?")
                .bind(folder_id)
                .fetch_all(&self.pool)
                .await?;
        Ok(rows.into_iter().map(|r| r.0).collect())
    ...
    pub async fn delete_email_by_uid(&self, folder_id: &str, uid: &str) -> Result<()> {
        ...
        let email_id: Option<(String,)> =
            sqlx::query_as("SELECT id FROM emails WHERE folder_id = ? AND remote_id = ?")
                .bind(folder_id)
                .bind(uid)
    ...
    pub async fn has_any_system_roles(&self, account_id: &str) -> Result<bool> {
        let count: (i64,) = sqlx::query_as(
            "SELECT COUNT(*) FROM folders WHERE account_id = ? AND system_role IS NOT NULL",
        )
        .bind(account_id)
        .fetch_one(&self.pool)
        .await?;
    ...
    pub async fn set_shortcuts(&self, shortcuts: &str) -> Result<()> {
        let value: serde_json::Value =
            serde_json::from_str(shortcuts).context("Invalid shortcuts JSON")?;
        if !value.is_object() {
            bail!("Shortcut config must be a JSON object");
        }
        let normalized = serde_json::to_string(&value).context("Failed to serialize shortcuts")?;
    ...
    pub async fn record_search_history(&self, account_id: &str, query: &str) -> Result<()> {
        ...
        let max_row: (Option<i64>,) =
            sqlx::query_as("SELECT MAX(last_used_at) FROM search_history WHERE account_id = ?")
                .bind(account_id)
                .fetch_one(&self.pool)
                .await?;
    ...
    pub async fn get_accounts_detailed(&self) -> Result<Vec<AccountInfo>> {
        let rows: Vec<(String, String, Option<String>, String, i64, bool, String, i64, bool, bool, i64, Option<i64>, String, Option<String>)> = sqlx::query_as(
            "SELECT id, email, display_name, imap_host, imap_port, imap_use_tls, smtp_host, smtp_port, smtp_use_tls, sync_enabled, sync_interval, last_sync, status, last_error FROM accounts"
        )
        .fetch_all(&self.pool)
        .await?;
...
mod tests {
    ...
    async fn test_db_initialization() {
        ...
        assert!(column_names.contains(&"sync_enabled".to_string()));
        assert!(column_names.contains(&"sync_interval".to_string()));
        assert!(column_names.contains(&"last_sync".to_string()));
        assert!(column_names.contains(&"status".to_string()));
        assert!(column_names.contains(&"last_error".to_string()));
    }
    ...
    async fn test_settings_defaults_seeded() {
        ...
        let confirm_delete = db.get_setting(SETTING_CONFIRM_BEFORE_DELETE).await.unwrap();
        let download_directory = db.get_setting(SETTING_DOWNLOAD_DIRECTORY).await.unwrap();
        let remote_image_policy = db.get_setting(SETTING_REMOTE_IMAGE_POLICY).await.unwrap();
        let search_history_limit = db.get_setting(SETTING_SEARCH_HISTORY_LIMIT).await.unwrap();
        ...
        assert_eq!(confirm_delete.as_deref(), Some("true"));
        assert_eq!(download_directory.as_deref(), Some(""));
        assert_eq!(remote_image_policy.as_deref(), Some("ask"));
        assert_eq!(search_history_limit.as_deref(), Some("10"));
    ...
    async fn test_upsert_logic() {
        ...
        let account = db
            .get_account_by_email("test@test.com")
            .await
            .unwrap()
            .unwrap();
        assert!(account.sync_enabled);
        assert_eq!(account.sync_interval, 15);
        assert_eq!(account.status, "normal");
    ...
    async fn test_multi_account_isolation() {
        ...
        let folder1 = db
            .upsert_folder(&acct1, "INBOX", "Inbox", 0, None)
            .await
            .unwrap();
        let folder2 = db
            .upsert_folder(&acct2, "INBOX", "Inbox", 0, None)
            .await
            .unwrap();
    ...
    async fn test_unread_count_calculation() {
        ...
        let acct_id = db
            .upsert_account("count@test.com", None, "h", 993, true, "h", 465, true)
            .await
            .unwrap();
        let folder_id = db
            .upsert_folder(&acct_id, "INBOX", "Inbox", 0, None)
    ...
    async fn test_folder_ordering_prioritizes_system_roles() {
        ...
        let acct_id = db
            .upsert_account("order@test.com", None, "h", 993, true, "h", 465, true)
            .await
            .unwrap();
        db.upsert_folder(&acct_id, "CUSTOM", "Custom", 0, None)
            .await
            .unwrap();
        db.upsert_folder(&acct_id, "SPAM", "Spam", 0, Some("SPAM"))
            .await
            .unwrap();
        db.upsert_folder(&acct_id, "INBOX", "Inbox", 0, Some("INBOX"))
            .await
            .unwrap();
        db.upsert_folder(&acct_id, "ARCHIVE", "Archive", 0, Some("ARCHIVE"))
            .await
            .unwrap();
        ...
        assert_eq!(
            ordered,
            vec!["INBOX", "SENT", "DRAFTS", "SPAM", "TRASH", "ARCHIVE", "CUSTOM"]
        );
    }
    ...
    async fn test_delete_email_by_uid_cleans_attachments() {
        ...
        let acct_id = db
            .upsert_account("delete@test.com", None, "h", 993, true, "h", 465, true)
            .await
            .unwrap();
        let folder_id = db
        ...
        let attachment_count: (i64,) =
            sqlx::query_as("SELECT COUNT(*) FROM attachments WHERE email_id = ?")
                .bind(&email_id)
                .fetch_one(&db.pool)
                .await
                .unwrap();
    ...
    async fn test_email_list_has_attachments_flag() {
        ...
        let acct_id = db
            .upsert_account("list@test.com", None, "h", 993, true, "h", 465, true)
            .await
            .unwrap();
        let folder_id = db
            .upsert_folder(&acct_id, "INBOX", "Inbox", 0, Some("INBOX"))
    ...
    async fn test_detail_cache_columns_roundtrip() {
        ...
        let acct_id = db
            .upsert_account("cache@test.com", None, "h", 993, true, "h", 465, true)
            .await
            .unwrap();
        let folder_id = db
            .upsert_folder(&acct_id, "INBOX", "Inbox", 0, Some("INBOX"))
    ...
    async fn test_search_filters_by_sender_and_attachments() {
        ...
        let email_id = db
            .upsert_email(&folder_id, &email_with_attachment)
            .await
            .unwrap();
        ...
        db.upsert_email(&folder_id, &email_no_attachment)
            .await
            .unwrap();
