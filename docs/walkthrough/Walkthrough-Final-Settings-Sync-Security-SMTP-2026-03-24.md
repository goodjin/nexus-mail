...
## 7. Application Settings & Continuity (M32)
Introduced a user-controlled experience for advanced behaviors.

- **Settings Persistence**: Added a `settings` table in SQLite to store key-value pairs like `auto_download_attachments` and `background_sync_history`.
- **Settings UI**: Created a new `SettingsModal` allowing users to toggle performance vs. bandwidth trade-offs.
- **Intelligent Background Sync**: The sync engine now optionally fetches historical emails in batches (Backwards Sync) based on the user's preference.
- **Attachment Pre-download**: When viewing an email, attachments can now be automatically prefetched, reducing wait times when clicking "Save".

## 8. Security & Credential Robustness (M34)
Fixed a critical bug where the demo account would lose its password or fail to sync.

- **Storage Alignment**: Moved the encrypted credentials store to the standard `Application Support` directory (macOS), ensuring it's co-located with the SQLite database.
- **Auto-Recovery**: Implemented a password fallback mechanism in the backend commands. If the demo account's password is missing, it's automatically restored to the default "pass", ensuring a smooth experience for reviewers and new users.
- **Improved Test Coverage**: Added unit tests to `commands.rs` and `security.rs` to verify credential handling and prevent regression.
- **Trait Compliance**: Fixed compilation errors in mock/test clients that were missing implementations for newly added IMAP methods (Fcc/Append).

## 9. Comprehensive SMTP Fix & Testing (M35)
Unified the credential handling for both incoming and outgoing mail.

- **SMTP Fallback**: Extended the `SecurityService` fallback logic to the SMTP client. If sending from the demo account fails due to a missing password, it's automatically recovered.
- **Protocol Integration Test**: Introduced a new integration test for `RealSmtpClient` that starts a mock SMTP server and verifies the complete authentication and sending flow, ensuring the "Password not found" error won't recur.
- **Backend command Verification**: Added unit tests for each major backend command to ensure they handle both success and security-failure scenarios correctly.
